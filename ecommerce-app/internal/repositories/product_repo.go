...
func (r *SQLServerProductRepository) GetByID(id string) (*models.Product, error) {
	...
	query := `SELECT CONVERT(CHAR(255),Id) AS 'char', Name, Description, Price, Stock, CreatedAt, UpdatedAt
...
func (r *SQLServerProductRepository) GetAll() ([]models.Product, error) {
	rows, err := r.db.Query(`SELECT CONVERT(CHAR(255),Id) AS 'char', Name, Description, Price, Stock, CreatedAt, UpdatedAt FROM Products`)
	if err != nil {
...
func (r *SQLServerProductRepository) Create(product models.Product) error {
	query := `INSERT INTO Products (ID, Name, Description, Price, Stock, CreatedAt, UpdatedAt)
...
func (r *SQLServerProductRepository) Update(product models.Product) error {
	query := `UPDATE Products
	          SET Name = @p1, Description = @p2, Price = @p3, Stock = @p4, UpdatedAt = @p5
	          WHERE ID = @p6`
	res, err := r.db.Exec(query,
		product.Name, product.Description, product.Price,
		product.Stock, time.Now(), product.ID,
	)
	if err != nil {

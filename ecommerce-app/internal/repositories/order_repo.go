...
type OrderRepository interface {
	...
	GetDB() *sql.DB
	GetOrdersByClientID(clientID string) ([]models.Order, error)
}
...
func (r *SQLServerOrderRepository) GetOrdersByClientID(clientID string) ([]models.Order, error) {
	query := `SELECT CONVERT(CHAR(255),o.ID) AS 'char',CONVERT(CHAR(255),o.ClientID) AS 'char', c.Name AS ClientName, o.OrderDate, o.TotalAmount, o.Status, o.CreatedAt, o.UpdatedAt
			  FROM Orders o
			  JOIN Clients c ON o.ClientID = c.ID
			  WHERE o.ClientID = @p1` // Filtrar por ClientID

	rows, err := r.db.Query(query, clientID)
	if err != nil {
		return nil, fmt.Errorf("error al obtener pedidos para el cliente %s: %w", clientID, err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			log.Printf("Error al escanear fila de pedido para cliente %s: %v", clientID, err)
			return nil, fmt.Errorf("error al escanear fila de pedido: %w", err)
		}
		orders = append(orders, o)
	}
	if err = rows.Err(); err != nil {
		return nil, fmt.Errorf("error en la iteración de filas de pedidos para cliente %s: %w", clientID, err)
	}
	return orders, nil
}
